    ...
    <div className="min-h-screen p-6 max-w-4xl mx-auto space-y-6 text-right">
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          {tasks.map((task) => {
            const isCustom = (task as any).task_page_type === "custom";
            return (
              <Card key={task.id} className={isCustom ? "bg-primary/5 border-primary/20" : "bg-card"}>
                    ...
                    <div className="flex-1 min-w-0">
                      {renamingId === task.id ? (
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {isCustom && <Badge variant="secondary" className="text-xs">ייחודית</Badge>}
                        </div>
                      )}
                      {renamingId !== task.id && (
