...
import type { HalachmimResult } from "@/lib/halachmim-data";
...
interface Props { taskId: string; taskTitle: string; }

function parseResults(answerText: string): HalachmimResult[] {
  try { return JSON.parse(answerText).matches || []; } catch { return []; }
}
...
const HalachmimDashboard = ({ taskId, taskTitle }: Props) => {
  ...
  const formatTime = (iso: string) => new Date(iso).toLocaleString("he-IL", { hour: "2-digit", minute: "2-digit", day: "2-digit", month: "2-digit" });
  const formatMinutes = (s: number) => `${Math.round(s / 60)} דקות`;

  const handleCopyLink = (submissionId: string) => {
    navigator.clipboard.writeText(`${window.location.origin}/task/${taskId}/feedback/${submissionId}`);
  ...
  const handleGradeChange = async (id: string, value: string) => {
    ...
    if (error) toast({ title: "שגיאה בשמירת הציון", variant: "destructive" });
    else queryClient.invalidateQueries({ queryKey: ["submissions", taskId] });
  };

  const handleDelete = async (id: string) => {
    const { error } = await supabase.from("submissions").delete().eq("id", id);
    if (error) toast({ title: "שגיאה במחיקה", variant: "destructive" });
    else { queryClient.invalidateQueries({ queryKey: ["submissions", taskId] }); toast({ title: "ההגשה נמחקה" }); }
  };
      ...
      <div className="space-y-4">
          ...
          {results.map((r, i) => (
            <div key={i} className={`rounded-lg border p-3 ${attemptsBg(r.attempts)}`}>
              <div className="flex items-center justify-between mb-1">
                <span className="font-semibold text-foreground text-sm">{r.blend}</span>
                <span className={`text-xs font-bold ${attemptsColor(r.attempts)}`}>
                  {r.attempts === 1 ? "ניסיון ראשון ✓" : `${r.attempts} ניסיונות`}
                </span>
              </div>
              <p className="text-xs text-muted-foreground">{r.word1} + {r.word2}</p>
              {!r.correct && <p className="text-xs text-destructive mt-1">לא נענה נכון</p>}
            </div>
          ))}
        </div>
        <div className="flex flex-wrap items-center gap-4 pt-2 border-t border-border">
          ...
          <div className="flex items-center gap-2">
            ...
            <Input dir="ltr" type="number" min={0} max={100} defaultValue={s.grade ?? ""} className="w-20 h-8 text-center text-sm"
              onBlur={(e) => handleGradeChange(s.id, e.target.value)} onKeyDown={(e) => e.key === "Enter" && (e.target as HTMLInputElement).blur()} />
          </div>
          ...
          <AlertDialog>
            <AlertDialogTrigger asChild>
              <Button variant="outline" size="sm" className="gap-1.5 text-destructive hover:text-destructive"><Trash2 className="h-3.5 w-3.5" />מחיקה</Button>
            </AlertDialogTrigger>
            <AlertDialogContent dir="rtl">
              ...
              <AlertDialogFooter className="flex gap-2 sm:justify-start">
                ...
                <AlertDialogAction onClick={() => handleDelete(s.id)} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">מחיקה</AlertDialogAction>
              </AlertDialogFooter>
            </AlertDialogContent>
          </AlertDialog>
        </div>
  ...
  return (
    <div className="min-h-screen p-6 max-w-4xl mx-auto space-y-6 text-right" dir="rtl">
      ...
      {isLoading ? <p className="text-muted-foreground">טוען...</p> : sorted.length === 0 ? <p className="text-muted-foreground">אין הגשות עדיין.</p> : (
              ...
              <AccordionItem key={s.id} value={s.id} className="border rounded-lg px-4 bg-card">
                <AccordionTrigger className="hover:no-underline">
                  <div className="flex flex-wrap items-center gap-4 text-sm w-full">
                    ...
                    <span className="text-muted-foreground flex items-center gap-1"><Clock className="h-3.5 w-3.5" />{formatTime(s.submitted_at)}</span>
                ...
                <AccordionContent>
                  <div className="pt-2 pb-4 border-t border-border mt-2">{renderSubmissionContent(s)}</div>
                </AccordionContent>
              </AccordionItem>
            );
          })}
...
export default HalachmimDashboard;
